...
export function VideoSelect(props: VideoUploadProps) {
  ...
  async function getVideoFile(): Promise<File | Blob | null> {
    ...
    if (youtubeUrl) {
      ...
      const response = await api.get(`/download/${videoId}`, { responseType: 'blob' });
    ...
    <form onSubmit={handleUploadVideo} className="space-y-4">
      <Tabs defaultValue="upload" className="flex flex-col">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="upload" disabled={!isWaiting()}>Upload</TabsTrigger>
          <TabsTrigger value="youtube" disabled={!isWaiting()}>Youtube</TabsTrigger>
        </TabsList>
